...
"component-byk-resql" "component-byk-ruuter" "component-byk-tim" "component-databases" "byk-bot")
components_charts=(
  ...
  "byk-bot" "../../Kubernetes/Components/Bot"
...
if [[ -z $selected_pods ]]; then
    ...
    if [[ "${non_running_components[*]}"  =~ "byk-bot" ]]; then
       echo `helm install byk-bot $(get_chart byk-bot "${components_charts[@]}") --namespace ${selected_namespaces[@]}`;
    fi
